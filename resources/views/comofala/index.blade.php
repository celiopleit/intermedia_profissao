...
<div class="card-body">
    <div class="card" style="box-shadow: 10px 10px 20px rgba(212, 108, 108, 0.3); transform: perspective(1000px) rotateX(10deg); transition: transform 0.3s;">
        <div class="row row text-center"  style="margin-top: 1rem; display: flex; justify-content: center;">
            <h2 style="transform: rotateX(-1deg);">Cadastro Habilidades com Idioma</h2>
        </div>
    </div>
</div>
   ...
   @endif
    ...
    <div class="card-body">
        <div>
           <a class="btn btn-success btn-sm " href="{{ route('comofala.create') }}">Novo</a>
        </div>
    ...
    <table class="table table-bordered  ">
        ...
        <tr>
            ...
            @foreach ($comofala_all as $key => $value)
                <tr>
                    <td>{{ $value->id }}</td>
                    <td>{{ \Str::limit($value->descricao) }}</td>

                    <td class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
                        <form action="{{ route('comofala.destroy', $value->id) }}" method="POST">
                            <a class="btn btn-primary btn-sm " href="{{ route('comofala.edit',$value->id) }}">Editar</a>
                            @csrf
